...
export default function CodeBlock({
        ...
        <div className="pr-8 relative overflow-y-auto">
          <Highlight
            ...
            lineNumberContainerProps={{
              className: 'pl-2 float-left left-0 sticky bg-code-block',
              style: {},
            }}
            lineNumberProps={lineNumber => {
              const isHighlighted = highlightedLines && highlightedLines.includes(lineNumber)
              return {
                className: `${isHighlighted ? 'bg-code-block-highlight text-gray-500' : 'text-gray-600'} block pl-2 pr-2`
              }
            }}
            wrapLines={true}
            lineProps={lineNumber => {
              const isHighlighted = highlightedLines && highlightedLines.includes(lineNumber)
              return {
                className: `${isHighlighted ? 'bg-code-block-highlight block ml-4' : ''} pr-8`,
              }
            }}
            codeTagProps={{
              className: 'mr-8'
  ...
  return (
    <>
      <div className={`${className} relative max-w-full rounded overflow-y-hidden overflow-x-auto p-2 bg-gradient-to-r from-code-block to-gray-600 z-10`}>
        {hasWindow && (
        ...
        {
          showCopy && (
            <div className={`${!showLineNumbers && codeBlocks[activeBlock].code.split('/n').length < 2 ? 'absolute top-0 bottom-0 right-0 pl-5 pr-2' : ''} z-10`}>
              <button onClick={onClickCopy} className="absolute z-50 text-xs text-white right-2 top-1 cursor-pointer hover:text-gray-300 focus:outline-none" title="Copy to clipboard">
